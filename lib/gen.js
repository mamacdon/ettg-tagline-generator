...
const corpusLines = corpusFiles.reduce((acc, path) => {
    return acc.concat(fs.readFileSync(path, "utf-8")
    .split(/\r?\n/))
    .filter(l => l !== "" && l[0] !== "#");
}, []);
...
module.exports = class Generator {
    ...
    string(options) {
        ...
        const limit = options.limit || randInt(10, 35);
        let result;
        while (true) {
            let array = this.genFn(limit);
            if (!array) {
                continue;
            }
            array = words.cleanGrammar(array);
            result = array.join(" ");
            if (words.words(result).length < 3) {
                continue; // scrap it
            }
